...
class CosineSimilarityTraceMiner:
    ...
    def __init__(
        ...
        mining_mode = str(mining_mode).strip().lower()
        if mining_mode not in {"single_pass", "sequential"}:
            raise ValueError("mining_mode must be 'single_pass' or 'sequential'.")
        self.model = model
    ...
    def _read_hidden_sequence(self, state: Dict[str, Any]) -> torch.Tensor:
        """Run one backbone pass and return the final-layer hidden sequence."""
        with torch.no_grad():
            outputs = self.model.backbone(
                inputs_embeds=state["inputs_embeds"],
                attention_mask=state["attention_mask"],
        ...
        return self.model._extract_final_hidden(outputs)
    ...
    def _patch_actions(
        ...
        return actions, patch_indices, cosine_scores

    def _record_similarity(self, cosine_scores: List[float]) -> None:
    ...
    def _single_pass_trace(
        ...
        for step in steps:
            ...
            boundaries.append((before_step, after_step, step_token_count))

        hidden_sequence = self._read_hidden_sequence(state)
        trace: List[Dict[str, Any]] = []
        decisions: List[Dict[str, Any]] = []
        for step_idx, (before_step, after_step, step_token_count) in enumerate(boundaries):
            step_hidden = hidden_sequence[:, after_step - 1, :]
            actions, patch_indices, cosine_scores = self._patch_actions(step_hidden, bank["patches"])
            decisions.append(
                {
                    ...
                    "sequence_length_after_actions": after_step,
                }
            )
            trace.extend(copy.deepcopy(actions))
            self._record_similarity(cosine_scores)
    ...
    def mine_example(self, example: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.mining_mode == "single_pass":
            trace, decisions = self._single_pass_trace(state, bank, steps)
        ...
        for step_idx, step in enumerate(steps):
            ...
            patch_actions, patch_indices, cosine_scores = self._patch_actions(step_hidden, bank["patches"])
