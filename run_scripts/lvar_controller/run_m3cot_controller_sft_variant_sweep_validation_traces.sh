...
OUTPUT_ROOT="${OUTPUT_ROOT:-outputs/controller_sft_m3cot_validation_sweeps}"
TRAIN_ROOT="${TRAIN_ROOT:-${OUTPUT_ROOT}/train}"
MANIFEST_PATH="${MANIFEST_PATH:-${OUTPUT_ROOT}/controller_runs.jsonl}"
GLOBAL_TRACE_PATH="${GLOBAL_TRACE_PATH:-outputs/oracle_dataset/m3cot/validation/lvar_ckpt/m3cot_validation_traces_lvar_global.jsonl}"
# COARSE_TRACE_PATH="${COARSE_TRACE_PATH:-outputs/oracle_dataset/m3cot/validation/lvar_ckpt/m3cot_validation_traces_lvar_coarse.jsonl}"
...
run_suite_for_trace_source() {
  ...
  run_one "${trace_source}" "${trace_path}" "multihot_binary_unweighted_unnormalized" "false" "true" "binary"
}
