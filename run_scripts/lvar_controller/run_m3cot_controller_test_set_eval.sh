...
EVAL_ROOT="${EVAL_ROOT:-${OUTPUT_ROOT}/eval_test}"
LIMIT="${LIMIT:-}"
...
while IFS=$'\t' read -r trace_source variant config_path checkpoint_path; do
  ...
  output_path="${output_dir}/m3cot_test_predictions.jsonl"
  mkdir -p "${output_dir}"

  if [[ "${SKIP_EXISTING}" == "true" && -s "${output_path}" ]]; then
    echo "Skipping existing test output: trace_source=${trace_source} variant=${variant} path=${output_path}"
    continue
  fi

  echo "Evaluating controller on test: trace_source=${trace_source} variant=${variant}"
...
echo "Done. Test outputs are under ${EVAL_ROOT}"
