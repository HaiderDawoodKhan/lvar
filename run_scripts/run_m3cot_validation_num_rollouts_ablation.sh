...
echo "Running sampled-controller rollout-count ablation"
echo "  partition: ${DATASET_PARTITION}"
echo "  rollouts:  ${ROLLOUT_VALUES}"
echo "  temp:      ${TEMPERATURE}"
echo "  seed:      ${SEED}"
echo "  output:    ${OUTPUT_ROOT}"

for num_rollouts in ${ROLLOUT_VALUES}; do
  if ! [[ "${num_rollouts}" =~ ^[0-9]+$ ]] || [[ "${num_rollouts}" -le 0 ]]; then
    echo "Invalid rollout value: ${num_rollouts}" >&2
    exit 2
  fi
  ...
  echo
  echo "Running num-rollouts ablation point G=${num_rollouts}"
...
echo
echo "Done. Num-rollouts ablation outputs are under ${OUTPUT_ROOT}"
