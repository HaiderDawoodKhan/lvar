...
eval_mined_traces() {
  ...
  for trace_variant in "${variants[@]}"; do
    # local inference_dir="outputs/inference/m3cot/test_oracle/mined_by_${mined_by_key}_ckpt/evaluated_by_${evaluated_by_key}_ckpt/trace_variant_${trace_variant}" LVAR_USE_MROPE_POSITION_IDS=1
    local inference_dir="outputs/inference/m3cot/test_oracle_mrope/mined_by_${mined_by_key}_ckpt/evaluated_by_${evaluated_by_key}_ckpt/trace_variant_${trace_variant}"
...
echo "Done. Existing trace datasets were read from outputs/oracle_dataset/test. Self and cross evals are under outputs/inference/m3cot/test_oracle/mined_by_*_ckpt/evaluated_by_*_ckpt."
