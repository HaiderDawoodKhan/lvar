...
run_one() {
  ...
  local trace_dir="outputs/oracle_dataset/train/${model_key}_ckpt"
  local trace_path="${trace_dir}/m3cot_train_traces_${model_key}_${context_label}.jsonl"

  mkdir -p "${trace_dir}"

  echo "Mining ${model_key} checkpoint on M3CoT train with ${context_label} context..."
...
echo "Done. Trace datasets are under outputs/oracle_dataset/train."